...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const { payment_status, amount_paid, note, next_due_at, access_status, access_expires_at } =
      await request.json();
    ...
    // Course access validity (ongoing/completed + expires_at) is separate
    // from payment status/installments — it only starts the first time a
    // payment is confirmed as "paid", and only once, so re-verifying or
    // correcting an already-paid enrollment never pushes the deadline out.
    let accessData =
      payment_status === 'paid' && !before.expires_at
        ? { status: 'ongoing', expires_at: calculateExpiryDate(before.course) }
        : {};

    // Manual admin override — e.g. extending a student's access or marking
